...
export function EditExpenseModal({ expense, plants, getUrlForKey, onClose, onUpdate }) {
  ...
  const [relatedPlantId, setRelatedPlantId] = useState(expense.relatedPlantId || "");
  const [tags, setTags] = useState(expense.tags || []);
  const [photoKeys, setPhotoKeys] = useState(expense.photos || []);
  const [previewUrls, setPreviewUrls] = useState({});
  const [loading, setLoading] = useState(false);
  const [oldPhotoKeys, setOldPhotoKeys] = useState(expense.photos || []);
    ...
    <Modal title="编辑花费" onClose={onClose}>
      <div className="space-y-3 max-h-[70vh] overflow-y-auto">
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <Button disabled={!category.trim() || !amount} onClick={handleSave}>
            保存
          </Button>
        </div>
      </div>
